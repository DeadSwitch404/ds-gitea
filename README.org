...
- Sets up a reverse proxy with SSL using Nginx (Install it with =ds_nginx=).
...
2. (Optional) Set up the PostgreSQL user and database
3. (Optional) Set up an =nginx= reverse proxy with SSL support
4. Set up default rate limiting when =nginx= is used
5. Create a user and group for the service
6. Create the required directory structure
7. Wait for the operator to save the secrets in SOPS or Ansible Vault (only if secrets are not present)
8. Deploy the Gitea =app.ini= configuration
9. Deploy the Gitea systemd service
...
| gitea_nginx_ratelimit            | string  | Nginx rate limit (requests per sec)                     |
| gitea_nginx_burst                | integer | Nginx burst for the reate limit                         |
...
| gitea_actions_enabled            | boolean | Enable or disable the Gitea Actions feature             |
